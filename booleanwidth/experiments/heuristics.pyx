...
def compute_data(input_dir):
    ...
    output_file = 'greedy_lboolc_raw_results'
...
def compute_avg_data(input_dir):
    data = {}
    results_file = 'greedy_lboolc_raw_results'
    ...
    with open(input_dir + 'greedy_lboolc_values', 'w') as f:
        for p, value in avg_data:
            f.write('{}:{}\n'.format(p, value))


def plot_data(input_dir):
    filenames = ['greedy_lboolc_values']
    data = []
